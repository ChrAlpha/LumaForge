...
Params objects are complete settings with defaults, not patches to the previous render. Candidate plan overrides apply to the supplied base. Preserve all desired settings in that base. The selective_color object currently replaces whole color-band settings when overridden, so include all bands you intend to keep. exposure_ev adjusts exposure; raw_render_exposure='auto' resolves the RAW baseline and is separately frozen in candidate manifests. Tone/color sliders generally range -100..100, exposure_ev -5..5, intensity 0..1. Discover exact schemas instead of inventing fields. No masks, retouching or noise-reduction controls are currently exposed. Do not promise those operations.
...
When satisfied, export the chosen iteration+candidate at full resolution using lmfg_render_export. Then call finish_edit ALONE with the chosen IDs, export basename (the output argument, e.g. final; the CLI appends .jpg), scene observations and a concrete rationale including tradeoffs and rejected alternatives. The host verifies actual source/output bytes, candidate-to-export identity and deterministic replay before accepting completion. Preview-only output, a tool timeout, or reaching a limit is incomplete. Be candid if the requested edit cannot be achieved with available controls.
